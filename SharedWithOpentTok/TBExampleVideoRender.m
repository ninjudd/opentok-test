...
- (EAGLContext *)context {
  if (_sharedContext == nil) {
    ...
    [EAGLContext setCurrentContext:_sharedContext];
...
+ (void) useContext {
  ...
  if ([EAGLContext currentContext] != [[OTGLContext sharedContext] context]) {
    [EAGLContext setCurrentContext:[[OTGLContext sharedContext] context]];
    glDisable(GL_DEPTH_TEST);
  }
}
...
- (void)setFrame:(CGRect)frame {
  ...
  NSLog(@"%s frame: %@", __PRETTY_FUNCTION__, NSStringFromCGRect(frame));
}
...
- (BOOL)initializeBuffers
{
  ...
  [[OTGLContext sharedContext].context renderbufferStorage:GL_RENDERBUFFER
  ...
  if(ret != GL_FRAMEBUFFER_COMPLETE) {
    ...
    CVReturn err = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, NULL,
                                                [OTGLContext sharedContext].context, NULL,
                                                &videoTextureCache);
    if (err) {
...
// This is "create" method, it's up to the client to call CVRelease when
// object not used anymore.
...
{
  if (_currentlyBackgrounded) {
    return;
  }
  
  CVPixelBufferRef newPixelBuffer = [self createPixelBufferWithFrame:frame];
  
  if (_snapshotBlock) {
    [self performSnapshot:newPixelBuffer];
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    size_t frameWidth = CVPixelBufferGetWidth(newPixelBuffer);
    size_t frameHeight = CVPixelBufferGetHeight(newPixelBuffer);
